...
4.  uMngeni Catchment IAPS data from EI4WS project INR SANBI and uMngeni Water
...
umbb <- resample(um, niaps, method = "near" )
```
...
inv_max_sum <- inv_max_tb %>%
  ...
  mutate(prp_inv = lc_8/ext_rem) %>%
   left_join(eoo, "T_MAPCODE") %>%
  mutate(B1i_invmax = case_when(EOO <= 2000 & prp_inv >= 0.3 ~ "CR", 
                                  EOO <= 20000 & prp_inv >= 0.3 ~ "EN", 
                                  EOO <= 50000 & prp_inv >= 0.3 ~ "VU", 
                                  TRUE ~ "LC")) %>%
  mutate(B2i_invmax = case_when(AOO <= 2 & prp_inv >= 0.3 ~ "CR", 
                                  AOO <= 20 & prp_inv >= 0.3 ~ "EN", 
                                  AOO <= 50 & prp_inv >= 0.3 ~ "VU", 
                                  TRUE ~ "LC")) %>%
  mutate(D1_invmax = case_when(prp_inv >= 0.8 ~ "CR",
                   prp_inv >= 0.5 ~ "EN",
                   prp_inv >= 0.3 ~ "VU",
                  TRUE ~ "LC")) %>%
  mutate(D3_invmax = case_when(prp_inv >= 0.9 ~ "CR",
                   prp_inv >= 0.7 ~ "EN",
                   prp_inv >= 0.5 ~ "VU",
                  TRUE ~ "LC"))
...
inv_strict_sum <- inv_strict_tb %>%
  ...
  mutate(prp_inv = lc_8/ext_rem) %>%
   left_join(eoo, "T_MAPCODE") %>%
  mutate(B1i_invstrict = case_when(EOO <= 2000 & prp_inv >= 0.3 ~ "CR", 
                                  EOO <= 20000 & prp_inv >= 0.3 ~ "EN", 
                                  EOO <= 50000 & prp_inv >= 0.3 ~ "VU", 
                                  TRUE ~ "LC")) %>%
  mutate(B2i_invstrict = case_when(AOO <= 2 & prp_inv >= 0.3 ~ "CR", 
                                  AOO <= 20 & prp_inv >= 0.3 ~ "EN", 
                                  AOO <= 50 & prp_inv >= 0.3 ~ "VU", 
                                  TRUE ~ "LC")) %>%
  mutate(D1_invstrict = case_when(prp_inv >= 0.8 ~ "CR",
                   prp_inv >= 0.5 ~ "EN",
                   prp_inv >= 0.3 ~ "VU",
                  TRUE ~ "LC")) %>%
  mutate(D3_invstrict = case_when(prp_inv >= 0.9 ~ "CR",
                   prp_inv >= 0.7 ~ "EN",
                   prp_inv >= 0.5 ~ "VU",
                  TRUE ~ "LC"))
# Write results  
