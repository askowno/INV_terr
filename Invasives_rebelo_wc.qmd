...
### Workflow for using Bioscape CFR IAT survey data in RLE assessments for Criterion D3
...
#add cfr invasive data path: 

cfr_fp <- list.files(path = data_path, 
                           pattern = "^BioScape_IAT_Map_Certainty\\.tif$",
                           recursive = TRUE,
                           full.names = TRUE)
...
cfr <- resample(cfr, lc2022, method = "near")
...
cert_cfr <- classify(cert_cfr, rcl = reclass_matrix)
...
inv_lc_veg <- crosstab(c(lc2022,vegrast,inv_cfr, cert_cfr), useNA=TRUE) #ensure NA areas included
