...
niaps_veg_sum <- niaps_lc_veg_tb %>%
 ...
 pivot_wider(names_from = sum_niaps, 
  ...
  mutate(niaps_ext30 = rowSums(across(c(niaps_ch1:niaps_ch10))/sumrem_sa)) %>%
  mutate(niaps_ext50 = rowSums(across(c(niaps_ch2:niaps_ch10))/sumrem_sa)) %>%
  mutate(niaps_ext70 = rowSums(across(c(niaps_ch5:niaps_ch10))/sumrem_sa)) %>%
  mutate(niaps_ext80 = rowSums(across(c(niaps_ch6:niaps_ch10))/sumrem_sa)) %>%
  mutate(niaps_ext90 = rowSums(across(c(niaps_ch7:niaps_ch10))/sumrem_sa)) %>%
  mutate(D3test = case_when(niaps_ext90 >= 0.9 ~ "CR",
                        niaps_ext90 >= 0.7 | niaps_ext70 >= 0.9 ~ "EN",
                        niaps_ext90 >= 0.5 | niaps_ext70 >= 0.7 | 
                        niaps_ext50 >= 0.9 ~ "VU",
                        TRUE ~ "LC")) %>%
  mutate(D1test = case_when(niaps_ext80 >= 0.8 ~ "CR",
                        niaps_ext80 >= 0.5 | niaps_ext50 >= 0.8 ~ "EN",
                        niaps_ext80 >= 0.3 | niaps_ext50 >= 0.5 | 
                        niaps_ext30 >= 0.8 ~ "VU",
                        TRUE ~ "LC")) %>%
    mutate(YEAR = 2023)
